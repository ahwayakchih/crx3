	...
	return new Promise((resolve, reject) => {
		...
		crxStream.once('finish', () => {
			...
			if (crxStream.crx.cfg.xmlPath) {
				xmlStream = writeUpdateManifestXML(crxStream.crx.cfg.xmlPath, crxStream.crx.encodedAppId, crxStream.crx.cfg.appVersion, crxStream.crx.cfg.crxURL);
			}
		});
	...
	return new Promise((resolve, reject) => {
		...
		crxStream.once('finish', () => {
			...
			if (crxStream.crx.cfg.xmlPath) {
				...
				xmlStream = writeUpdateManifestXML(crxStream.crx.cfg.xmlPath, crxStream.crx.encodedAppId, manifestData.version, crxStream.crx.cfg.crxURL);
			}
		});
		crxStream.once('close', () => (xmlStream && xmlStream.once('close', () => resolve(info))) || resolve(info));
 ...
 * @param {string} appId
 * @param {string} [version='${APP_VERSION}']
 * @param {string} [url='${CRX_URL}']
 */
function writeUpdateManifestXML (filePath, appId, version = process.env.APP_VERSION || '${APP_VERSION}', url = process.env.CRX_URL || '${CRX_URL}') {
